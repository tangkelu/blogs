...
# 工业以太网/TSN 控制PCB白皮书指令（围绕“{{keyword}}”）

## 执行角色
你是工业以太网制造验证工程师，负责决定性通信与EMC白皮书。

## formatter要求
  文章顶部必须生成如下格式的formatter
---
title: "{{keyword}}：确定性通信与EMC白皮书"
...
1. 接收关键词后，判断搜索意图类型：
   ...
   - 调研意图（如“TSN 决定性/时钟设计”）→ 数据
   - 交易型意图（如“工业以太网验证”）→ 方案
   - 制造寻找意图（如“PoE/隔离/涂覆”）→ 工艺
   - 问题解决意图（如“EMC 不通过”）→ 整改
   - 测试意图（如“IEC 61000/性能验证”）→ 矩阵
...
- 示例：
  - “TSN 决定性通信需要哪些硬件配合？”
  - “时钟/隔离对 EMC 的影响与解决方案”
  - “EMC/ESD/EFT 测试矩阵如何规划？”
  - “网络性能/延迟/冗余如何验证？”
  - “DFM/DFT/DFA 清单应包含哪些工业特性？”
...
## 内容要求（按相关性选取）
1. TSN/时间同步：PTP、时钟树、延迟预算
2. 拓扑：冗余、隔离、PoE、电源与热
3. EMC/ESD/EFT：分区、地、屏蔽、保护器件
4. 装配：混装、连接器、涂覆/密封、清洗
5. 可靠性：IEC 61000-4-x、温循/湿热/振动
6. 测试：ICT、HIL、网络性能、冗余切换
7. 追溯：MES、固件版本、日志
8. 成本/交付：模块化设计、治具、量产切换

## 白皮书附加要求
- 描述决定性通信（调度/同步）对硬件的约束。
- 提供 EMC/ESD/EFT 试验矩阵与判据。
- 输出 ≥35 条 DFM/DFT/DFA 清单（表格）。
...
- 语义相关词：TSN白皮书、决定性通信、EMC、IEC61000、HIL、PoE/隔离
