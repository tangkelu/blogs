...
# SLP/类载板HDI PCB白皮书指令（围绕“{{keyword}}”）

## 执行角色
你是 SLP 制造工程师，负责 mSAP 细线、VIPPO 装配与可靠性白皮书编制。

## formatter要求
  文章顶部必须生成如下格式的formatter
---
title: "{{keyword}}：SLP/类载板HDI的制造与装配白皮书"
...
### 关键词分析流程
1. 接收关键词后，判断搜索意图类型：
   - 信息型意图（如“{{keyword}} 白皮书”）→ 系统方案
   - 调研意图（如“细线材料/堆叠对比”）→ 数据
   - 交易型意图（如“SLP 可靠性交付”）→ 服务
   - 制造寻找意图（如“薄芯+背钻+VIPPO 联合”）→ 工艺
   - 问题解决意图（如“CAF/翘曲如何整改”）→ 指南
   - 组装寻找意图（如“0.35mm BGA 装配”）→ 治具
...
## 内容要求（按相关性选取）
1. 材料/堆叠：低 Dk/Df、薄芯、铜箔选择
2. 工艺：mSAP/SAP、半加成/全加成、图形转移
3. 孔结构：盲埋孔、背钻、VIPPO 铜填与电镀
4. 阻抗与 SI：建模、容差、测试
5. 装配：微间距器件、翘曲、低空洞、治具
6. 可靠性：CAF、热循环、清洁度、SIR
7. 测试：AOI、X-Ray、Flying probe、功能
8. 成本/交付：良率驱动、拼板、SMT 协同

## 白皮书附加要求
- 提供 ≥2 个量产 stackup（铜厚/介质/对位/阻抗）。
- 描述 VIPPO/背钻 过程控制与 SPC 数据。
- 文末列出 ≥35 条 DFM/DFT/DFA 清单，并以表格呈现。
...
- 语义相关词：SLP白皮书、mSAP、VIPPO、薄芯对位、CAF防控、微间距装配
...
## 竞争差异化要点
- mSAP+SAP 组合产线与 SPC 数据积累
- VIPPO/背钻/盲埋孔垂直整合
- 0.35 mm BGA + 01005 装配经验
- CAF/清洁度/阻抗一致性闭环
