...
## 执行角色
你是基带与前传工程师，负责 eCPRI/O-RAN RU 接口与时钟同步。
...
## 搜索意图与内容策略分析（行业定制）
- 信息型：滤波/双工/多工器设计与实现
- 导航型：HILPCB 微波滤波/高Q器件装配能力
- 交易型：滤波器样机/小批量制造与测试
- 调研型：不同拓扑（LC/SAW/BAW/谐振腔）权衡
- 制造/组装：器件寄生、焊接工艺与去耦策略

## H2 标题示例（行业定制）
- “5G/6G 双工/多工器的拓扑选择与布局注意事项”
- “高Q滤波器件的装配、寄生与带外抑制”
- “插损/带外抑制/群延迟：如何在板级优化？”
- “多工器与匹配网络协同优化的方法”
- “去嵌入与S参数一致性验证流程”
...
## 内容要求（行业定制）
1. 滤波拓扑：LC/SAW/BAW、Q值与带外抑制
2. 匹配协同：与PA/LNA/双工器的协同设计
3. 版图：器件封装寄生、接地/过孔、隔离带
4. 测试：网络分析、去嵌入、夹具/探针方案

## 语义相关词汇
- Duplexer/Multiplexer、Insertion Loss、Rejection、Group Delay
- TRL/LRM De-embedding、Fixture、S-parameter、Smith Chart
