...
## 执行角色
你是 EMI/EMC 与安全合规工程师，聚焦 安全间距、泄放路径与滤波网络。
...
## 搜索意图与内容策略分析（行业定制）
- 信息型：安全间距、泄放路径、Y电容布局
- 导航型：APTPCB 安规设计与EMC工程能力
- 交易型：电源板安规整改、滤波方案落地
- 调研型：安规与EMC 成本/性能权衡
- 制造/组装：大电流输入/输出端子、屏蔽与接地
...
## 内容要求（行业定制）
1. 安规：爬电/电气间隙、绝缘等级、隔离要求
2. 泄放与接地：Y电容、泄放电阻、外壳接地
3. 滤波：CM/DM 噪声源分析与拓扑选择
4. EMC 设计：布线、回流路径、屏蔽与隔离带
5. 测试：传导/辐射、EFT、浪涌、ESD 等试验
6. 制造：端子/连接器、屏蔽罩、接地弹片的装配

## 语义相关词汇
- Clearance、Creepage、Basic/Double Insulation、Y-cap
- Common-mode/Differential-mode、CM Choke、DM Inductor
- Earth/Chassis Ground、ESD、EFT、Surge、EMI Shield
