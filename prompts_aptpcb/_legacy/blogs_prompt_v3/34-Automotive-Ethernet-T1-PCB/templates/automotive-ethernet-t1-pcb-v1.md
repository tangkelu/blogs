...
# 车载以太网T1 PCB内容生成指令（围绕“{{keyword}}”）

## 执行角色
你是车载以太网硬件与EMC工程师，负责 T1 物理层、PoDL、EMC 与验证。

## formatter要求
  文章顶部必须生成如下格式的formatter
---
title: "{{keyword}}：驾驭车载以太网T1 PCB的EMC与一致性挑战"
description: "解析{{keyword}}的差分阻抗/回流路径、磁件与连接器、EMC/ESD/浪涌与可靠性，确保车规量产。"
category: technology
date: "{{date}}"
featured: true
image: ""
...
### 关键词分析流程
1. 接收关键词后，判断搜索意图类型：
   - 信息型意图（如“{{keyword}} 差分/阻抗”）→ 教育
   - 导航型意图（如“APTPCB 车载以太网能力”）→ 实力
   - 交易型意图（如“T1 线束/PCB 制造”）→ 方案
   - 问题解决意图（如“EMC 不过怎么改”）→ 诊断
   - 制造寻找意图（如“PoDL 高压设计”）→ 工艺
   - 验证意图（如“ISO 11452/CISPR 25”）→ 测试
...
## 内容要求（按相关性选取）
1. 差分设计：阻抗、回流、编织效应缓解
2. 磁件/连接器：布局、屏蔽、接地、护套
3. PoDL/隔离：供电与数据共线、热与安全
4. EMC：分区、接地、屏蔽、保护器件
5. 装配：清洗、涂覆/密封、连接器应力
6. 验证：AEC-Q100、ISO 11452、CISPR 25、ESD/EFT
7. 可靠性：温度循环、振动、湿度与长寿命
8. 追溯：条码、MES、软件/固件配合
...
- 语义相关词：车载以太网、100BASE-T1、1000BASE-T1、PoDL、EMC、ISO11452、CISPR25、AEC-Q
