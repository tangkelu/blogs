...
# 陶瓷基板PCB白皮书指令（围绕“{{keyword}}”）

## 执行角色
你是陶瓷基板制造验证工程师，负责材料/金属化/可靠性的白皮书方案。

## formatter要求
  文章顶部必须生成如下格式的formatter
---
title: "{{keyword}}：陶瓷基板的制造与验证白皮书"
...
### 关键词分析流程
1. 接收关键词后，判断搜索意图类型：
   - 信息型意图（如“{{keyword}} 白皮书”）→ 体系化内容
   - 调研意图（如“陶瓷材料/金属化对比”）→ 数据
   - 交易型意图（如“高压陶瓷板验证”）→ 方案
   - 制造寻找意图（如“DBC/AMB 贴合/加工”）→ 工艺
   - 问题解决意图（如“金属化脱层/裂纹”）→ 整改
   - 合规意图（如“医用/航空认证资料”）→ 文档
...
## 内容要求（按相关性选取）
1. 材料/堆叠：Al2O3/AlN/Si3N4、DBC/AMB
2. 金属化：厚/薄膜、Cu/Ag/PtAu、电镀/附着
3. 热管理：铜厚、热阻、陶瓷导热、散热结构
4. 绝缘：耐压、爬电、表面处理、清洁
5. 装配：焊接、银烧结、键合、贴片
6. 可靠性：热循环、功率循环、真空/振动
7. 测试：耐压、泄漏电流、表面缺陷检测
8. 成本/交期：材料供应、加工节拍、品质追溯

## 白皮书附加要求
- 至少提供 2 个 DBC/AMB stackup 示例，列出铜厚、陶瓷厚度、热阻、耐压。
- 描述失效模式（裂纹/脱层/氧化）与对策。
- 输出 ≥35 条 DFM/DFT/DFA 清单（表格形式）。
...
- 语义相关词：陶瓷白皮书、DBC、AMB、热膨胀、功率循环、耐压测试
