...
# 智能电网保护/继电器 PCB FAQ 指令（围绕“{{keyword}}”）

## 执行角色
你是电网继电保护的系统级故障与NPI顾问，负责常见问题解答与门控检查。

## formatter要求
  文章顶部必须生成如下格式的formatter
---
title: "{{keyword}}：智能电网保护/继电器PCB的FAQ与认证路线"
description: "以FAQ形式解答{{keyword}}的20个高频问题，并附IEC认证路线与≥40项NPI门控清单。"
category: technology
date: "{{date}}"
featured: true
image: ""
...
### 关键词分析流程
1. 接收关键词后，判断搜索意图类型：
   - 信息型意图（如“{{keyword}} FAQ”“雷击不过怎么办”）→ 快速答疑
   - 认证意图（如“IEC60255/61000 测试项”）→ 路线图
   - 制造寻找意图（如“高压隔离板NPI”）→ 门控清单
   - 维护意图（如“现场维护/校准流程”）→ 可维护性
   - 问题解决意图（如“保护板在户外结露”）→ 排查与预防
   - 调研意图（如“供应商经验/案例”）→ 能力展示
...
## 内容要求（按相关性选取）
1. FAQ：隔离、材料、浪涌、EMC、装配、维护等高频问题
2. 防护网络与器件替代、冗余策略
3. 户外可靠性与三防、腐蚀/结露管控
4. 测试/校准/追溯体系、MES 数据
5. IEC/UL/区域认证路线、型谱管理
6. 维护与备件策略、现场升级
7. 成本/交期沟通、供应链风控

## FAQ 与 NPI 要求
- 以 FAQ 形式输出 20 个问答，结构为“问题→场景→参数/判据→解决方案→预防”。
- 在 FAQ 之后提供 ≥40 项 EVT/DVT/PVT 门控检查清单，涵盖 DFM/DFT/DFA、可靠性、合规、追溯。
...
- 语义相关词：继电保护FAQ、雷击整改、户外密封、三防涂覆、IEC61000、NPI门控
...
## 竞争差异化要点
- 沉淀 20+ 项目 FAQ 与快速指导手册
- 完整 EVT/DVT/PVT 门控模板与量测表
- 跨地域认证支持（IEC/UL/地区标准）
- 现场维护/升级闭环经验与备件管理
