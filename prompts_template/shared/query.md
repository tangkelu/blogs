# Query Prompt

你要输出一篇可直接发布的 PCB / PCBA 技术博客，文章类型为 `Query`，围绕主关键词：`{{keyword}}`，输出语言必须与传入 locale 一致。

这篇文章的目标是：

- 直接回答一个明确的工程问题、制造问题、选型问题、成本问题、交期问题或排查问题
- 帮助读者完成设计、选型、制造、装配、验证或供应商沟通决策
- 让内容更容易被搜索系统、AI 搜索和问答系统抽取

你会收到或预先准备以下输入：

- 主关键词：`{{keyword}}`
- 日期：`{{date}}`
- 标签：`{{tags}}`
- evidence pack：包含公开事实、项目级判断、不可写边界；其事实层默认优先来自 `llm_wiki`
- 目标站点 overlay：HILPCB 或 APTPCB
- 必要时加载的模块：`comparison`、`application`、`capability`

严格执行以下要求。

## 输出要求

1. 只输出最终 Markdown。
2. 首字符必须是 `---`。
3. 不输出分析过程、推理过程、提示词解释或写作说明。
3.1 不输出任何内部思考痕迹，例如“我如何得出结论”“为什么这样组织内容”“按模板/提示词/工作流执行”。
4. 不写 AI 口吻，不写“本文将”“接下来我们”之类空泛导语。
5. 语言要像工程团队或制造团队写给工程师、采购、项目经理看的技术文档。
6. 先回答问题，再解释背景，再展开细节。
7. 结构合格不代表事实合格；如果标题、description 或小标题承诺了某个标准、参数、频段、材料、测试方法或 capability 主题，正文必须兑现，不能只给泛化概述。

## 数据边界

### 可以写成公开事实

- 标准组织、行业联盟、监管机构、官方 datasheet、白皮书、官方工艺说明中的事实
- 公开可验证的定义、术语、测试方法、合规框架、材料属性、公开工艺原理

### 只能写成项目级判断

- DFM 优先级
- 试产或量产中的风险排序
- 什么时候更适合选 A 或 B
- 检查项、验证路径、控制带、评审优先级
- 供应商沟通重点、报价所需资料、试产建议

### 禁止写入

- 编造数字、阈值、良率、交期、案例结果
- 编造工厂 capability、认证、设备、产能、客户结果
- 把项目经验写成行业统一标准
- 把站点产品页描述包装成全行业事实

如果 evidence pack 没有给出数字，就不要补数字；改写成条件判断、影响因素或验证方法。

## 博客重写 / 生成前置契约

在生成正文前，必须先按 `blog-rewrite-data-gap-contract.md` 执行一次前置判断。该判断是硬门槛，不是建议。

默认流程：

1. 先拆解博客或关键词的关键点：搜索意图、工程决策、必须解释的概念、需要的表格 / 检查表 / 选择框架、站点承接动作、不可写 claim。
2. 先检查 `llm_wiki` 是否已有足够 source / fact / wiki / gate 支撑顶尖稿；默认优先消费本地知识库，不先直接查外部来源。
2.1 执行本地检查时，默认先用 `rg` 在 `llm_wiki/sources/registry/`、`llm_wiki/facts/`、`llm_wiki/wiki/`、相关 `llm_wiki/logs/` 中搜索主关键词、同义词、标准号、材料名、工艺名和失败模式词；只有没有 `rg` 时才回退到 `grep`。
2.2 对抽象工程主题，默认还要先把文章归到 `failure mechanism family`，并优先消费 `llm_wiki/wiki/processes/blog-failure-pattern-mechanism-family-map.md`。如果连机制族都说不清，就不要开始正文。
3. 如果 `llm_wiki` 缺少关键事实，才允许补外部来源；补完后必须先回写 `llm_wiki`，再继续正文。
4. 如果状态是 `safe_but_generic`、`needs_data` 或 `hold`，不能直接写正文；必须先补内部证据库或缩小文章承诺。
4.1 如果现有证据只能写出一篇“安全但薄”的 Query 文章，也必须停下，不能先输出保守版本充数；此时应把状态视为 `safe_but_generic` 或 `needs_data`。
5. 只有状态达到 `ready`，或数据缺口已补齐并形成 evidence pack 后，才能继续执行正文生成。
5.1 在开始正文前，必须先形成一份最小消费清单：
   - 本次正文直接消费的 `fact_id`
   - 本次正文直接消费的 `wiki` 页面
   - 本次正文对应的 `failure mechanism family`
   - 仍需去官方来源补的缺口
   - 补完后要回写到 `llm_wiki` 的目标文件类型
   - 支撑这些消费项的本地 `rg` 命中路径或等价检索结果
   如果没有这份清单，或清单只有泛化 source 名称而没有事实消费路径，应停止生成。
5.2 如果本地 `llm_wiki` 已经提供了可用的检查项、边界、失效模式、参数语境或验证动作，正文必须优先消费这些本地材料；外部来源只能补缺口，不能替代本地消费。
5.3 正文的每个核心 H2 最好都能对应至少一条本地消费路径；如果某个 H2 只能依赖外部来源而本地完全没有命中，应先回到 `llm_wiki` 补源或缩小该 H2 承诺。
6. 重写时必须消费新增数据，不能补了数据但正文仍停留在泛泛解释。
6.1 如果主题属于 `SI`、`DFM`、`validation`、`stackup`、`materials`、`cost`、`lead time`、`reliability`、`connector / launch`、`test / ICT / fixture / probe / DFT` 之一，正文不得只剩抽象 review posture；至少要补足 `1` 段典型工程场景、常见失效模式、EQ 触发点或 release-review burden 说明。
6.1.0 这类主题在写正文前，必须先从本地 taxonomy 里选出主 `failure mechanism family`，并按该机制族去组织 failure pattern；不能只因为“有一个案例”就跳过机制归类。
6.1.1 如果主题涉及 `ICT`、`test points`、`bed-of-nails`、`flying probe`、`fixture`、`probing`、`support tooling` 或其它会向板子施加载荷的测试路径，正文相关 H2 里必须优先消费本地 `llm_wiki` 中关于 `mechanical load / board flex / latent damage` 的事实卡；如果本地没有这类卡，状态不能判成 `ready`，必须先补官方来源并回写知识库。
6.1.2 如果主题涉及 `stackup / SI / PI / impedance / RF / EMC / ESD`，正文 failure pattern 不得只写 `review burden`；默认至少要落到 `return path / coupling / field path / protection path` 其中一条真实失控链。
6.1.3 如果主题涉及 `thermal / power / LED / heavy copper / MCPCB`，正文 failure pattern 默认至少要解释 `heat path / thermal accumulation / CTE mismatch` 中的一条。
6.1.4 如果主题涉及 `assembly / stencil / reflow / via-in-pad / warpage / solderability`，正文 failure pattern 默认至少要解释 `process window 被压窄` 的具体方式，而不是只说“量产有风险”。
6.1.5 如果主题涉及 `quote package / release docs / data exchange / Gerber / ODB++ / IPC-2581 / BOM completeness`，正文 failure pattern 默认可以不是物理损坏，但必须明确 `缺什么输入 -> 谁暂停 -> 为什么误做或拖期` 这条治理失效链。
6.2 删除 unsupported numerics 之后，必须用工程动作、失败链路、trade-off、冻结点或供应商沟通重点补回信息密度；不能只把原文压缩成安全版提纲。
6.3 生成每个主体 H2 前，先做一次“主题词替换测试”：如果把当前主题替换成相邻主题后，这一节大部分句子仍然成立，说明内容还不够特异，应继续下钻到机制、失效模式、材料层级、结构卡点或验证动作。
6.4 FAQ 不得主要由防御性免责声明构成；如果问题是 `Does this article prove X?`，回答中必须继续解释真正该看什么、该如何确认、边界在哪里。
6.5 参数密集型主题的标准写法是“先找参数，再写参数边界”：如果正文需要数字、标准目标、测试条件或材料参数，必须同时写明它属于什么方法、什么场景、什么边界，不能只给裸数字。

禁止把“结构完整、没有明显错误”当作顶尖稿合格标准。Query 文章必须给出可执行的工程判断、选择逻辑、检查项或验证动作。

## Evidence Pack 强制执行

在生成正文前，必须按 `evidence-pack-consumption-contract.md` 执行一次 evidence pack 消费检查。该检查是硬门槛，不是建议。

### 状态处理

| Evidence status | 允许写法 | 禁止写法 |
| --- | --- | --- |
| `verified` | 可写成公开事实；数值只能在该 fact 自身已验证且不属于 hard refusal class 时使用 | 不得扩展成 pack 未授权的相邻参数、能力表或阈值 |
| `framing_only` | 只能写成定性背景、风险语境、流程判断或项目级建议 | 不得升级成数字、标准阈值、能力上限、验收结论或供应商证明 |
| `blocked` | 只能作为删除、拒写或内部 `DATA_GAP` 信号 | 不得进入正文、FAQ、表格、标题或 description |
| `must_refresh` | 刷新前视为不可用 | 不得写入正文、表格、FAQ 或来源区 |
| `supplier_scoped_dated_only` | 只能在保留供应商、日期和适用范围的专门语境中使用 | 不得提升为共享行业事实、HILPCB/ATPPCB 通用能力或可迁移供应商证明 |

### Hard Refusal Classes

以下 claim classes 默认禁止写入，除非当前 evidence pack 明确给出更窄的 `verified` 支撑，并且该支撑没有被标成 `supplier_scoped_dated_only`：

- shared `B` capability numerics：`trace/space`、最小机械钻孔、最小激光孔、aspect ratio、annular ring、impedance tolerance、registration tolerance 及类似 capability table 数字
- standards thresholds：`IPC Class 2 / 3 / 3A`、`IPC-6012 / 6013`、`IST`、`PLT`、coupon plan、plating minima、bow/twist limits、acceptance threshold 及类似验收阈值
- supplier proof：supplier approval、qualification、compliance、release authority、accepted-lot status、可迁移工厂能力证明
- process windows / recipes：lamination-count allowance、stack recipe、bake / press cycle、build-up default、manufacturability window
- high-speed / RF budget numerics：channel-loss budget、insertion-loss table、Nyquist budget、RF geometry rule、性能预算表
- dynamic commercial numerics：lead time、quick-turn window、yield、cost uplift、MOQ、区域或客户商业数字

### 缺证据时的强制行为

如果某个句子、表格行、FAQ、标题承诺或模块需要 `verified` 支撑但 evidence pack 没有提供，必须按顺序处理：

1. 如果已有 `framing_only` 支撑，降级为定性判断、验证动作或项目级建议。
2. 如果无法安全降级，删除该句子、表格行、FAQ 或模块。
3. 如果缺口会影响文章结构完整性，在工作检查中标记 `DATA_GAP`，并缩小标题、description 或 H2 承诺。

禁止用相邻事实、行业常识、标准名称、供应商页面、workflow 名称或旧稿数字补齐缺口。

### 内部控制信息不得进入正文

Evidence pack 里的 `Disposition`、`Prompt action`、`Current-blog section bridge map`、`Unsupported / Unclear`、`Not cleared`、`do not write`、`downgrade`、`delete`、`blocked`、`DATA_GAP` 等内容只能用于生成前判断，不能原样或改写后进入最终文章。

最终正文应把这些控制信息转换为读者可用的工程表达：

- 把“不要怎么写 / 不应写什么”改成“项目风险信号 / 需要确认什么 / 适用边界是什么”
- 把“删除某类数字”改成“该参数需按具体 stackup、材料和 DFM 确认”
- 把“blocked claim”改成不出现该 claim，或改成面向项目的验证动作
- 表格列名不得出现 `不应怎么写`、`禁止写入`、`Prompt action`、`Disposition`、`Evidence status`、`DATA_GAP` 等内部审稿语义
- H2 / H3 不得暴露“模板、审稿、策略、证据包、生成规则”这类内部流程
- 作者与审核信息不得出现内部证据库名称、内部文件路径、内部数据层、内部审核 workflow 或任何仓库/提示词系统名称；应改成“工程内容审核团队”“制造工程团队”“材料与工艺审核团队”等对外可理解实体
- 正文、脚注、FAQ、表格、作者信息、review 信息、引用说明中，都不得出现 `llm_wiki`、`evidence pack`、`source layer`、`local evidence layer`、`working prompt`、`prompt`、`workflow`、`internal`、`repo`、`knowledge base` 等内部执行术语
- 正文不得出现任何内部思考过程措辞，例如 `analysis`、`reasoning`、`chain-of-thought`、`my logic`、`I chose this structure because`
- 最终成稿不得使用 Markdown 脚注语法，例如 `[^1]`、`[^validation]`；来源归因只能写成正文内联来源句柄或括号式说明

在开始写正文前，默认再做一次快速自检：

- 标题承诺了什么
- description 承诺了什么
- 哪些 claim 属于公开事实
- 哪些 claim 只能写成项目级判断
- 哪些地方必须放正文内联来源归因
- 哪些标准号、版本号、日期、频段或材料家族名称必须再核对一次
- 哪些 claim 的 evidence status 不是 `verified`，需要降级、删除或标记 `DATA_GAP`
- 是否存在 hard refusal class 被写入标题、description、表格、FAQ 或正文
- 是否把 evidence pack 的内部控制语句误转成了正文栏目、表格列名或小标题
- 作者与审核信息是否完全移除了内部证据库名称、内部路径、内部 workflow 和提示词系统名称
- 正文与脚注是否完全移除了 `llm_wiki`、`evidence pack`、`source layer`、`local evidence layer`、`working prompt`、`prompt`、`workflow`、`internal`、`repo`、`knowledge base` 等内部执行术语
- 正文是否完全移除了“我如何思考 / 我如何判断 / 为什么这样写”的元叙述
- 正文是否完全移除了 Markdown 脚注语法，例如 `[^1]`、`[^validation]`

## 文章必须包含的结构

- frontmatter
- 唯一 H1 大标题
- H1 下方顶部答案块
- 一个可被 AI 单独摘录的定义型摘要 / quick answer block
- 至少一张早期规则 / 参数 / 对比表
- 2-4 个围绕关键词意图展开的主体 H2
- FAQ
- 公开参考资料
- 与站点解决路径相关的下一步
- 目标站点 overlay 要求的内置表单、询盘或 RFQ 组件
- 作者与审核信息

## Frontmatter

必须输出如下结构，并替换占位符：

```yaml
---
title: "<主关键词前置的查询式标题>"
description: "一句话回答搜索意图，并说明本文覆盖哪些规则、风险、验证方法。"
category: technology
date: "{{date}}"
featured: true
image: ""
readingTime: 8
tags: {{tags}}
---
```

## H1 规则

- frontmatter 之后必须紧跟一个且仅一个正文 H1，大标题内容默认与 frontmatter.title 保持一致或高度一致
- 不要把 frontmatter.title 当作可见 H1 的替代
- H1 必须出现在正文前部，不能缺失，也不要在后文再出现第二个 H1
- 如果有首图，允许顺序为 `frontmatter -> 首图 -> H1 -> 顶部答案块`
- 如果没有首图，默认顺序为 `frontmatter -> H1 -> 顶部答案块`

## 标题规则

- 标题必须把主关键词尽量前置。
- 标题要匹配查询意图，优先使用 `What Is`、`How to`、`X vs Y`、`Cost`、`Lead Time`、`Troubleshooting`、`Design Rules` 一类表达。
- 批量重写同一批博客时，禁止连续复用同一个标题骨架，尤其不要反复使用 `X Guide: What Matters in ...`、`X Design Guide: ...` 这类只替换关键词的句型。
- 同一批 Query 标题应主动做句式分散，可在 `What Is`、`How to Review`、`When to Use`、`X vs Y`、`Design Priorities`、`Common Mistakes`、`Validation Checklist` 等结构之间切换。
- 不写空泛品牌标题，不写新闻标题，不写夸张标题党。
- 标题不要承诺正文无法兑现的内容。
- 如果标题写了 `rules`、`design rules`、`validation`、`cost`、`lead time`、`aspect ratio`、`FR1/FR2`、`materials` 这类强 promise，正文必须给出对应的规则语境、验证路径、驱动因素或来源支撑，不能只给泛化判断。

## 正文结构

### 0. 批量结构轮换硬规则

同一批 Query 文章不能连续复用同一套 section order，也不能连续复用相同 intro、CTA 或 failure-chain 句式。

生成或重写同一批文章时，必须在以下正文结构中轮换；如果上一篇已使用某结构，下一篇必须换用另一种结构，除非关键词意图明显不匹配，并且要同步改变小标题措辞：

| Structure ID | Section order | 适用主题 | 必须避免 |
| --- | --- | --- | --- |
| `QA-decision-review` | `Quick answer -> Decision table -> Failure mechanism -> Review checklist` | DFM、NPI、validation、release readiness、review 型主题 | 不要连续复用“first check / then review / finally freeze”句式 |
| `QA-boundary-release` | `Quick answer -> What it is not -> Physical boundary -> Release implications` | 概念澄清、SI / PI / thermal / materials 边界、容易误解的工程术语 | 不要把每篇 intro 都写成“X is not just Y” |
| `QA-mistake-freeze` | `Quick answer -> Common mistake -> Why it fails -> What to freeze` | failure、troubleshooting、test、assembly、制造风险主题 | 不要连续复用“missing input -> EQ -> delay”同一链路句式 |
| `QA-selection-handoff` | `Quick answer -> Selection criteria -> Trade-off table -> Handoff` | 材料、工艺路线、供应商沟通、Gerber / ODB++ / BOM / test handoff 主题 | 不要把 handoff 结尾都写成“send the package through the quote page” |

批量改写时必须执行以下反套壳规则：

- `section order`：同一 lane 相邻两篇不得使用同一 `Structure ID`
- `intro`：相邻两篇不得使用同一承接句式，例如连续使用 “If your ... is moving into prototype or pilot build”
- `CTA`：相邻两篇不得使用同一 CTA mode opening、同一资料包排序或同一 quote / review 收口句
- `failure-chain`：相邻两篇不得复用同一位置、长度和句式；可放入主体 H2、side note、caution block 或 release checklist，但全文只保留 `1` 条主 failure chain

### 1. 顶部答案块

紧跟 H1 输出。

要求：

- 用 `4-6` 条短结论直接回答核心问题
- 每条结论都要可执行、可判断或可验证
- 这一段必须足够独立，单独被截取时也能回答问题
- 默认直接以短结论列表输出，不强制增加 `## 直接回答` 这样的可见小标题
- 如果主题确实需要过渡性小标题，可以使用更自然的工程标题，但不要机械重复“直接回答”模板字样

### 1.5 定义型摘要 / Quick Answer Block

在 `顶部答案块` 之后，必须增加一个适合 AI 抽取的短摘要块。

要求：

- 用 `40-60` 词左右的单段或单卡片直接定义核心问题
- 必须能脱离上下文独立成立
- 优先覆盖“它是什么 / 何时需要 / 最关键判断依据”
- 不要写空泛品牌话术，不要重复整段前言
- 如果 quick answer 含有标准、频段、材料、测试方法、参数或公开技术结论，必须确认这些说法在正文和来源中都能落住。

### 2. 目录

长文默认启用目录。

要求：

- 当全文存在 `3` 个及以上主体 H2，或正文已经明显超过短答文章长度时，必须添加目录
- 目录必须放在 `顶部答案块` 与定义型摘要块之后、正文主体之前
- 目录项优先对应主体 H2、FAQ、结语、作者与审核信息等主要锚点
- 目录文本要可读，不要机械重复完整长标题

### 3. `{{keyword}}` 在工程上先看什么？

要求：

- 用一段话定义问题边界
- 明确说明它不等于什么，避免概念混淆
- 尽快告诉读者本问题最先看的 `3-5` 个维度

### 4. 关键规则与参数总表

必须尽早出现，优先使用以下列结构：

`规则 / 参数 | 推荐范围或判断方式 | 为什么重要 | 怎么验证 | 如果忽略会怎样`

如果主题是比较型，可改成：

`维度 | A 的表现 | B 的表现 | 什么时候更适合`

### 5. 第二层早期结构化信息块

如果主题属于以下任一类型，默认启用：

- SI
- DFM
- 材料
- 阻抗
- 背钻
- 可靠性
- EMC
- 连接器
- 容差控制
- 工艺窗口

这一层必须紧跟在第一张早期表格之后，使用以下二选一：

- 4-card 风格的信息块
- 额外一张或多张 Markdown table

### 5.5 典型工程场景 / Failure Pattern

当主题属于以下任一类型时，默认加入 `1` 条主 failure chain 或同等密度的典型工程场景，除非该主题本身是纯定义型 `knowledge` 页面且正文已经通过边界表、选择表和 FAQ 解释清楚风险：

- `SI`
- `DFM`
- `assembly`
- `validation`
- `stackup`
- `materials`
- `cost`
- `lead time`
- `reliability`
- `thermal`
- `high voltage / isolation`
- `connector / launch`

要求：

- 放在最相关的 H2、side note、caution block、release checklist 或 handoff section 中；不强制每篇放在同一位置
- 只写通用工程场景，不编造客户名、项目名、节省金额、良率结果或 capability 奇迹
- 必须写清：
  - 缺了什么输入、约束或边界
  - 触发了哪个 EQ、review pause、返工动作或验证 burden
  - 失控的物理机制、装配机制、测试盲区或数据包断点是什么
  - 为什么它会影响成本、进度、风险或 release
- 结论必须回扣到当前主题的冻结点、验证动作、review package 或 handoff package；不要每篇都使用“为什么不能只扔 Gerber / 只看走线 / 只看名义参数”同一句式
- 如果当前 evidence pack 不支持该主题的 numerics，就用定性的 failure chain 来写厚，不得强行补数字
- 全文只保留 `1` 条主 failure chain；其他风险只能做短提醒，避免段段都写成事故报告

### 6. 下一步 / CTA

必须保留一个与正文技术问题直接相关的 `Next steps` 或等效章节。

要求：

- CTA 必须按 `page_intent_mode` 选择，不得所有文章默认使用 quote / 24h DFM 收口
- `knowledge`：只给下一步阅读路径或概念延伸链接，不强行要求提交资料
- `review`：写成 review package，允许引导提交 `Gerber`、`stackup`、`BOM`、目标阻抗、测试要求、材料限制、应用约束、enclosure 条件等，并说明会返回的 review 输出；不默认写 SLA
- `handoff`：写成 handoff package，强调 Gerber / ODB++ / BOM / 坐标 / assembly drawing / test requirement / release note 的交接完整性和缺项后果
- `commercial`：才使用 quote path、公开邮箱、SLA 或 quick-turn / cost / lead-time 承接
- 不要把 CTA 写成只剩产品线链接的购物目录
- 可以在主 CTA 后补一行“如果其中某一部分仍未定义，可先看以下页面”，再放 `2-4` 个相关内链
- CTA 语气应像工程支持或制造协作，不要写成夸张销售文案
- 同一批文章不得连续复用同一 CTA 顺序；在 “pain -> package -> output -> contact”、 “knowledge gap -> next reading -> decision point”、 “handoff gap -> missing input -> owner -> release action” 等结构间切换
- 只有 `commercial` 模式且 overlay 已公开时，才写 `within 24 hours`、`same-day` 之类响应表述

### 7. 正文技术图的嵌入方式

如果正文要插入技术图、流程图、决策图或对比信息图，必须满足以下要求：

- 图必须服务于当前段落的解释任务，不能像横插进来的宣传图
- 是否需要插图，按以下优先级判定：
  - `must_add_figure`：正文正在解释工艺流程对比、测试 / 检验路径、结构分区或 stackup / finish zoning、失效模式与排查路径、设计决策矩阵或冻结点，且图能够把这层关系稳定翻译成视觉逻辑
  - `should_add_figure`：文章主题是 `cost`、`lead time`、`DFM`、`validation`、`routing`、`stackup`、`materials`、`reliability` 这类抽象工程流程，且图能把两个以上步骤、阶段或控制点压缩成更清楚的判断框架
  - `no_need`：图只会重复首图、重复表格，或只提升视觉丰富度而不能增加信息密度
- 插图前先用 `1-2` 句引导读者：这张图帮助看清什么变化、对比或决策逻辑
- alt 不能只写主题词；要写清图中比较的是哪两条流程、哪些变量或哪些阶段
- 图注必须解释：
  - 这张图想说明的工程结论
  - 哪些控制点被转移、放大或分开
  - 它不是在证明什么
- 如果图不能明显提高这一节的信息密度，就不要加
- Query 文章默认优先 `0-1` 张正文技术图，除非主题命中 `must_add_figure` 或天然需要第二张决策图
- 命中 `must_add_figure` 时，正文技术图默认不可省略
- 命中 `should_add_figure` 时，优先补图而不是留白

这一层的目标是尽早呈现：

- trade-off
- 工程约束
- 失效风险
- 验证路径

### 6. 主体 H2

从以下方向中按关键词意图选择 `2-4` 个，不要全部堆上：

- 设计 / layout 关键点
- 制造 / stackup / 工艺关键点
- 装配 / 测试 / 检验关键点
- 失效模式与排查
- how to choose / trade-off
- cost / lead time 驱动因素
- 应用场景差异
- 供应商确认项

每个主要 H2 必须遵守：

- 开头先给结论
- 再解释工程原因
- 再解释制造影响或商业影响
- 最后给验证方法、确认动作或沟通动作
- 如果该段包含具体数值、规范目标、材料参数、测试方法或典型结论，正文内必须加入自然的内联来源归因，例如“根据 USB-IF 规范”“NXP layout guide 指出”“TI 应用笔记给出”
- 如果该段提到标准号、标准版本、标准名称、频段定义、材料家族名、协议版本或测试方法编号，写入前必须先核对是否准确、是否为当前有效口径
- 不要把“文末有参考资料”当作“正文已被支撑”的替代；关键结论应在关键段落里被支撑
- 不要只用 bullets 代替论证；对复杂主题，至少要有一段连贯说明把机制、风险和动作串起来

不要写成纯科普段落，也不要只堆概念。

### 6.5 可选增强：HTML 卡片

当主题存在以下任一情况时，可以插入 `1` 组轻量 HTML 卡片：

- 需要在前 30% 内容里快速呈现 trade-off
- 需要把“适用场景 / 主要风险 / 推荐动作 / 验证路径”并列展示
- 纯 Markdown table 不够直观，但仍然需要保持专业感
- 图片不合适，或图片会重复首图 / 降低信息密度

对以下参数密集型或工程控制型主题，默认优先考虑 `早期 Markdown table + 4-card HTML 卡片`：

- SI
- DFM
- 材料
- 阻抗
- 背钻
- 可靠性
- EMC
- 连接器
- 容差控制
- 工艺窗口

如果主题本身更适合用多张表表达，则不要强行插入卡片，优先保留 `早期 Markdown table + 补充 Markdown table`。这类主题通常包括：

- 成本优化
- 优先级排序
- trade-off 决策矩阵
- make vs buy 判断
- 纯比较型 / 排名型内容

卡片使用规则：

- HTML 卡片是增强项，不是每篇强制项
- 单篇建议最多 `1` 组，避免模板味过重
- 卡片必须总结正文里已经成立的工程信息，不能为了凑视觉块再发明阈值
- 卡片文案必须表达工程结论、冻结项、风险信号、验证动作或选型提醒，不能写装饰性营销文案
- 卡片不是随机装饰；只有在确实能提升扫描效率时才使用
- 即使使用 HTML 卡片，也不能替代关键规则表和正文主体
- 如果 HTML 卡片已经完成了核心解释任务，就不要再额外补一张等价图片

卡片视觉规则：

- 如果同一关键词集群内多篇文章都使用卡片，不要机械复用同一套主色、渐变和视觉气质
- 配色必须服从主题逻辑，而不是随机换色
- 可参考以下方向：
  - 材料 / 表面处理 / 制程对比：偏材料感、工艺感、树脂 / 铜 / 表面处理色系
  - 功率电子 / 逆变器 / 高电流：偏工业能量、热管理、深色工业控制色系
  - 医疗 / 低噪声 / 高可靠性：偏洁净、克制、低饱和、医疗感色系
  - 高频高速 / 阻抗 / SI：偏信号完整性、层叠、传输路径感的冷静技术色系
- 视觉变化要克制，目标是“同一系统下的分组变化”，不是每篇乱换，也不是每篇都一样
- 无论使用什么配色，都要优先保证文字对比度、扫描效率和工程感
- 卡片内正文文字必须显式使用深色系，例如 `#111827`、`#1f2937`、`#334155`，禁止默认白字叠在浅色背景上
- 卡片标题、说明文、辅助注释都要保证在白底页面上可直接阅读，不得依赖背景图或渐变来“托字”

### 6.6 可选增强：典型工程场景 / EQ 模式

当主题属于以下任一类型时，可以补 `1` 段典型工程场景：

- `lead time`
- `cost`
- `DFM`
- `validation`
- `release`
- `test`
- `stackup`
- `materials`
- `failure / troubleshooting`

这类增强的目标是：

- 用一个典型工程场景解释前面已经成立的抽象规则
- 说明哪个输入缺失、哪个工程动作被迫暂停、为什么会拖慢交期或推高复杂度
- 增加信息密度和工程真实感，而不是注水扩写

写法规则：

- 默认只写 `1` 段，不单独强制开 `Case Study` 章节
- 优先插在相关 H2 里，通常放在规则表或结论段之后
- 用 `common failure pattern`、`typical scenario`、`common EQ trigger` 这类通用工程场景写法
- 不要写成客户成功故事或销售案例
- 不得编造客户名、项目名、节省金额、量产结果、良率数字或具体厂内窗口值
- 如果写了具体数值、工艺边界或标准窗口，仍然必须满足正文证据要求

### 7. 与站点解决路径相关的下一步

要求：

- 把技术判断自然连接到产品、服务、材料、能力、工具或 RFQ 动作
- 优先导向目标站点的产品页或服务页，博客页只能作为次级补充
- 内链分布要均衡，不要全部堆在开头或结尾
- 内链锚文本要语义自然，不要机械精确匹配堆砌
- 如果目标站点 overlay 定义了内置询盘、报价或 RFQ 组件，正文必须按 overlay 要求插入，不能只在文字里写“联系我们”或只放普通链接

### 8. 常见问题（FAQ）

必须保留以下包裹结构：

```html
<!-- faq:start -->
<!-- faq:end -->
```

FAQ 要求：

- 只放真实会被问到的补充问题
- 问题要短、准、可检索
- 问题优先使用自然语言搜索问句和检索式问法，贴近真实 query phrasing，而不是目录式标题
- 回答要直接，不要重复正文整段内容
- FAQ 应优先解决误区、边界和项目追问，不要把 FAQ 写成免责声明清单
- 如果 FAQ 使用否定句开头，后面必须继续给出工程判断、确认动作或正确的替代问题

### 9. 结语

用一小段总结：

- 什么时候该重点关注这个问题
- 什么时候需要进一步做 DFM、stackup、材料或装配确认

### 9.5 公开参考资料

正文结尾必须增加 `公开参考资料` 或 `Sources` 区块。

要求：

- 至少列出 `3` 条公开可点击来源
- 至少列出 3 条公开可点击来源
- 优先使用标准组织、官方 datasheet、官方设计指南、白皮书、官方技术文章
- 每条来源都要说明它支撑了正文中的哪个结论
- 不要只裸贴链接，格式要适合人工审查
- 如果正文写了具体数值、阻抗目标、材料参数或测试方法，参考资料区必须能回溯到这些结论
- 如果正文提到了标准号、标准名称、版本号、频段边界或方法编号，参考资料区必须能回溯到这些 identity claim，而不是只给泛化二级资料

### 10. 作者与审核信息

必须包含：

- 作者角色
- 技术审核角色
- 最近更新语义

优先要求：

- 如果站点有真实作者、审核人、工程负责人或技术团队实体页，优先使用真实可识别实体
- 如果暂时没有真实姓名，至少给出更具体的职能描述、团队边界和审核责任
- 作者与审核信息要让读者和 AI 系统理解这不是匿名营销文

## AI 可引用性要求

- 关键定义、规则、对比结论要能被 AI 直接摘录，不依赖上下文补全
- 正文不能只在文末列来源；关键结论所在段落应带内联来源归因
- 能写成结构化块的内容，优先写成表格、短列表、定义块、FAQ，而不是长段说明
- 如果主题涉及公开标准、datasheet、白皮书、layout guide、测试方法，必须显式体现出处类型
- 页面必须同时服务人类读者和 AI 系统，不要为了 SEO 牺牲可读性
- 去空泛时优先删除重复解释，不要删除已经有证据支撑的对比表、检查表、门控表、边界表或 FAQ

不要伪造真实姓名；如果没有明确姓名，用角色表述。

## 模块加载规则

### comparison 模块

当关键词属于 `X vs Y`、`difference between`、`which is better`、`how to choose A or B` 时，必须补充：

- 一张早期比较表
- 明确决策规则
- 明确例外边界

不能只写“看需求而定”，必须说明什么需求对应什么选择。

### application 模块

当关键词具有明显行业 / 终端应用属性时，补充：

- 行业 / 场景表
- 场景差异化约束
- 场景下的验证动作

不要把行业应用写成营销案例，要写成设计约束和制造要求差异。

### capability 模块

当关键词具有强商业 / 制造 / 报价 / MOQ / lead time 意图时，补充：

- 已公开 capability 快照
- 交期、MOQ、RFQ 或 DFM 所需资料
- 从技术问题走向询盘动作的自然过渡

限制：

- 只能使用站点已公开能力
- capability 内容不能压倒技术主体

## 写作规则

- 每个 H2 都要 answer-first。
- 优先使用表格、列表、短段落，不要写成大段密集长文。
- 对复杂主题，优先把参数、约束、判断条件提前，而不是埋在后文。
- 如果某个说法无法确认来源，就改写成“项目中通常需要确认”“应按供应商能力和验证结果判断”。
- 如果某个标题词或 H2 里的关键术语无法被正文充分兑现，应缩小标题承诺，而不是硬撑。
- 不要强制加入 glossary、supplier checklist、cheat sheet、行业应用表、capability 大表，除非关键词真的需要。
- 不要泄露“模块”“模板”“提示词”“overlay”“evidence pack”等内部术语。
- 如果 evidence pack 已经足够支撑结构化表达，正文至少应保留一组真正有判断价值的结构块，例如对比表、检查表、门控表、决策矩阵或边界表，而不是只剩概述段落

## 质量门槛

最终文章必须同时满足：

- 能让工程师在前 20% 内容里抓到核心判断
- 能让采购或项目经理看出该补什么资料、该确认什么风险
- 能让搜索系统抽取出定义、规则、参数、比较结论、FAQ
- 能自然承接到目标站点的商业页面
- 关键 technical claims 在事实边界上站得住，不因为“结构完整”而放松标准、版本、参数和来源核对

## 最终发布前自检

在输出最终 Markdown 前，默认逐条确认：

- 标题和 description 承诺的内容，正文是否逐项兑现
- 是否存在“宽词进、窄词出”或“强 promise、弱展开”
- 正文关键结论是否有对应的正文内联来源归因
- 文末参考是否真的能回溯到正文中的标准、版本、参数、测试方法和材料结论
- 是否把项目级判断误写成公开行业硬事实
- 是否存在标准号、版本号、日期、频段定义、材料家族名、测试方法编号写错或过时
- 是否因为缺数据而仍然硬补了数字、阈值或 capability
- 是否已经按 `verified / framing_only / blocked / must_refresh / supplier_scoped_dated_only` 完成 evidence pack 状态处理
- 是否已经删除或降级所有 hard refusal class，而不是把它们藏进表格、FAQ、卡片或 capability 模块
- 如果某个 claim 无法核实，是否已降级成条件判断、验证动作或项目级建议
- 目标站点 overlay 要求的内置表单、询盘或 RFQ 组件是否已经插入到正文合适位置，且没有放进 frontmatter、代码块、表格或 FAQ 包裹中

现在开始生成最终 Markdown 文章。
