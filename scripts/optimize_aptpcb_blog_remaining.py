...
def _extract_title_from_frontmatter(fm_text: str) -> str | None:
    ...
    for i, line in enumerate(lines):
        ...
        # Support folded/block scalar titles (common for long titles), and also
        # recover from previously-corrupted `title: '>-` cases by treating `>-`
        # as the block scalar indicator when followed by indented lines.
...
def _update_title_in_frontmatter(fm_text: str, *, title: str) -> str:
    ...
    while i < len(lines):
        ...
        if re.match(r"^title:\s*", line) and not replaced:
            ...
            replaced = True
            i += 1
            # Remove any indented continuation lines from folded/block titles.
...
def _ensure_h1(body: str, title: str) -> str:
    ...
    # Ensure the first non-empty content is an H1.
    i = 0
    ...
    # Remove any additional H1s that appear before the first H2 section.
    out: list[str] = []
...
def _read_changed_en_paths() -> set[str]:
    if not TRACKER_PATH.exists():
        return set()
    text = TRACKER_PATH.read_text(encoding="utf-8", errors="ignore")
    return set(re.findall(r"`(/code/hileap/frontendAPT/public/static/blogs/2025/\d\d/en/[^`]+\.md)`", text))


def _find_remaining_posts() -> list[PostRef]:
    changed_en_paths = _read_changed_en_paths()
    ...
    for month_dir in sorted(year_dir.iterdir()):
        ...
        for md in sorted(en_dir.glob("*.md")):
            if str(md) in changed_en_paths:
                continue
            posts.append(PostRef("2025", month, md.stem))
    return posts


def _append_tracker_entries(posts: list[PostRef]) -> int:
    existing = _read_changed_en_paths()
    added = 0
    lines: list[str] = []
    for post in posts:
        en_path = BLOGS_ROOT / post.year / post.month / "en" / f"{post.slug}.md"
        if str(en_path) in existing:
            continue
        lines.append(
            f"- `{post.slug}` → `https://aptpcb.com/en/blog/{post.slug}` (also `https://aptpcb.com/blog/{post.slug}`) → `{en_path}`"
        )
        added += 1
...
def main() -> int:
    posts = _find_remaining_posts()
    print(f"remaining_posts={len(posts)}")
    if not posts:
        return 0
    ...
    added = _append_tracker_entries(posts)
    print(f"updated_files={updated} missing_files={missing} skipped_files={skipped} unwrapped={unwrapped} tracker_added={added}")
