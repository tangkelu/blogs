...
TEMPLATE = """# PARAMETERS (filled by script at runtime)
# ----------------------------------------------------
# keyword: <string, 主关键词>
...
def generate_keywords(section):
    subs = []
    for name, words in section['keywords'].items():
        subs.append({
            'name': name,
            'keywords': [ {'keyword': w, 'used': False} for w in words ]
        })
    return {
...
def render_template(info):
    intent_block = fmt_list(info['intent_examples'])
    h2_block = fmt_list(info['h2_examples'])
    content_block = fmt_numbered(info['content_points'])
    variant_block = "\n" + info['variant_block'] + "\n" if info.get('variant_block') else ''
    competitive_block = fmt_list(info['competitive_points'])
    return TEMPLATE.format(
        doc_label=info['doc_label'],
        role=info['role'],
        formatter_title=info['formatter_title'],
        formatter_desc=info['formatter_desc'],
        intent_block=intent_block,
        h2_block=h2_block,
        content_block=content_block,
        variant_block=variant_block,
        seo_terms=info['seo_terms'],
...
def fmt_numbered(items):
    return '\n'.join(f"{i}. {item}" for i, item in enumerate(items, 1))

data = []  # TODO: populate new categories data here

if not data:
    print('No category data defined. Populate `data` list before running.')
    exit()

for section in data:
    section_dir = PROMPTS_DIR / f"{section['number']:02d}-{section['slug']}"
    section_dir.mkdir(parents=True, exist_ok=True)

    keywords_path = section_dir / 'keywords.json'
    keywords_path.write_text(json.dumps(generate_keywords(section), ensure_ascii=False, indent=2), encoding='utf-8')

    templates_dir = section_dir / 'templates'
    templates_dir.mkdir(exist_ok=True)

    for variant_name, variant_info in section['templates'].items():
        content = render_template(variant_info)
        (templates_dir / f"{section['slug'].lower()}-{variant_name}.md").write_text(content, encoding='utf-8')
