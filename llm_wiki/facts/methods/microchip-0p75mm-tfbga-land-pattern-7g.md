---
fact_id: "methods-microchip-0p75mm-tfbga-land-pattern-7g"
title: "Microchip 0.75 mm TFBGA values are reusable only as the named 169-ball 7G package drawing and land-pattern row"
topic: "Microchip 0.75 mm TFBGA land pattern for the named 169-ball 7G package"
category: "methods"
status: "verified"
confidence: "medium"
must_refresh: false
...
> Microchip's `169-Ball Thin Fine Pitch Ball Grid Array (7G)` package drawing is strong enough to support one additional narrow exact-data layer for the `PCB资料` package lane: the local corpus may reuse this named-package `0.75 mm` pitch row and printed `RECOMMENDED LAND PATTERN` data when the package identity and drawing context stay attached. This card is a vendor-scoped named-package drawing only. It does not authorize a universal `0.75 mm pitch -> pad diameter` rule.

## Exact Data Scope

- exact for:
  - Microchip's printed `169-Ball Thin Fine Pitch Ball Grid Array (7G) - 10x10x1.10 mm Body [TFBGA]` drawing
  - the printed `0.75 BSC` pitch for this named package
  - the printed `RECOMMENDED LAND PATTERN` value `Contact Pad Diameter (X169) b 0.35`
  - the printed package-mark boundary note for the `Terminal A1 visual index feature`
- not exact for:
...
- Treat this card as one owner-scoped package drawing for a named `169-ball` `7G` TFBGA package.
- Keep the `0.75 BSC` pitch and `Contact Pad Diameter` content attached to the Microchip package identity and drawing context.
...
- Use this card to show that more than one official `0.75 mm` owner row exists without claiming that every `0.75 mm` package shares the same land-pattern geometry.
- Pair this card with:
...
- It does not authorize package-library defaults outside the named `7G` package drawing.
...
- This card gives the package lane a second official owner-scoped replacement row for part of the handbook `0.75 mm` pitch pressure.
- It narrows the residual state from `one 0.75 mm owner row exists` to `multiple owner-scoped 0.75 mm rows exist`.
...
- authority replacement used here:
  - official Microchip package drawing `169L_TFBGA_10x10_7G_C04-377C-J`
- exact-data shape:
