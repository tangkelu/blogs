...
title: "Public IPC-6012 addendum metadata supports program-specific medical and space-military framing for rigid boards, not reusable acceptance tables"
...
source_ids:
  ...
  - "ipc-6012fs-toc"
tags: ["ipc", "ipc-6012", "ipc-6012em", "ipc-6012fs", "medical", "space", "military", "class-3", "metadata"]
---

# Canonical Summary

> Public IPC metadata is now strong enough to support a clearer rigid-board program hierarchy in `llm_wiki`: `IPC-6011A` as the generic printed-board framework for the broader `IPC-601X` family, `IPC-6012F` as the current rigid printed-board base specification, `IPC-6012EM` as a medical-applications addendum layer triggered by procurement documentation, and `IPC-6012FS` as a current space/military avionics addendum layer that modifies `Class 3`-adjacent production-lot and reliability contexts. These public sources still do not expose reusable acceptance thresholds or test tables.
...
- Use this card when a `22-layer` or other hi-rel rigid-board draft needs to distinguish generic `IPC-6012` / `Class 3` framing from program-specific medical or space/military addendum framing.
...
- Use this card to support wording such as `program-specific addendum`, `procurement-triggered requirements`, `space/military avionics addendum`, or `medical applications addendum`.
...
- It does not prove that any supplier is approved, compliant, certified, or qualified under `IPC-6012EM` or `IPC-6012FS`.
- It does not justify rewriting public addendum metadata into universal rules for all `22-layer` boards.

## Open Questions

- Add a follow-on fact only if a future batch finds more current public IPC metadata for other `IPC-6012` addendum families relevant to rigid high-layer boards.
